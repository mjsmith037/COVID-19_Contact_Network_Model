...
//[[Rcpp::export]]
IntegerMatrix run_disease_simulation_on_network(int maxtime,
                                                IntegerMatrix contact_network,
                                                IntegerVector initial_condition,
                                                LogicalVector vulnerable,
                                                NumericVector beta,
                                                double eta,
                                                double sigma,
                                                double rho,
  ...
  for (t=1; t<maxtime; t++) {
    ...
    for (i=0; i<npop; i++) {
      ...
      if (current_number_infectious != 0 && timeseries(i,t-1) == 0) {
        ...
        // if no connection has lead to infection, also check for background infection
        if (timeseries(i,t) == 0 &&
